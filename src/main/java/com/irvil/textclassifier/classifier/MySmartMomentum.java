 ...
 * 
 */
public class MySmartMomentum implements Strategy {
	 ...
	 */
	public static final double MAX_MOMENTUM = 50;
	
	/**
	 * The starting momentum.
	 */
	public static final double START_MOMENTUM = 0.01;
	
	/**
	 * How much to increase momentum by.
	 */
	public static final double MOMENTUM_INCREASE = 0.001;
		...
		if (this.ready) {
			...
			if ((this.lastImprovement > 0)
					|| (Math.abs(this.lastImprovement) 
							< MySmartMomentum.MIN_IMPROVEMENT)) {
				this.lastMomentum++;

				if (this.lastMomentum > MySmartMomentum.MOMENTUM_CYCLES) {
					this.lastMomentum = 0;
					if (((int) this.currentMomentum) == 0) {
						this.currentMomentum = MySmartMomentum.START_MOMENTUM;
					}
					this.currentMomentum *= 
						(1.0 + MySmartMomentum.MOMENTUM_INCREASE);
